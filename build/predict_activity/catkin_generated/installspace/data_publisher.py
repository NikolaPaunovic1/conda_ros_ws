...
class WatchClient:
    def __init__(self, local_port=6000):
        ...
        self.pub = rospy.Publisher('imu_data', Float64MultiArray, queue_size=1)
    ...
    def listen_for_messages(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Append data to list
            self.data_list = self.data_list + acc + gyr
            ...
            if len(self.data_list) == 40 * 6:  # We now have a sequence of 40 entries
                # Debugging block: Check the types of all elements
                for i, el in enumerate(self.data_list):
                    if not isinstance(el, float):
                        rospy.logerr(f'Element at index {i} is not a float, but {type(el)}: {el}')
